...
const IGNORED_DIRS = new Set(['node_modules', '.wrangler', '.git', 'dist', 'coverage', '.idea', '.vscode', 'reference/uploads']);
const IGNORED_NAMES = new Set(['.env', '.dev.vars', '.DS_Store', 'Thumbs.db']);
const IGNORE_EXT = new Set(['.db', '.db-shm', '.db-wal', '.sqlite', '.sqlite-shm', '.sqlite-wal', '.pem', '.key', '.p12', '.pfx', '.log', '.png', '.jpg', '.jpeg', '.gif', '.webp', '.svg']);
...
for (const f of files) {
  if (f.endsWith('formal_secret_scan.mjs')) continue; // 扫描器自身含模式串与旧标识符，属正常
  let c;
  ...
  lines.forEach((l, i) => {
    for (const [name, re] of patterns) {
      if (re.test(l)) {
        // 排除明显是占位符/示例/变量名声明/文档 export 示例/测试常量/官方域名 的行
        if (/example|your-|YOUR_|change-me|change_me|CHANGE_ME|REPLACE|xxxx|placeholder|示例|占位|TODO|dev-secret|admin123|^export\s|TEST_|TEST_PASSWORD|smoke-test|仅测试|weixin\.qq\.com|w3\.org|localhost|127\.0\.0\.1/.test(l)) continue;
        issues.push({ file: rel, line: i + 1, pattern: name, text: l.trim().slice(0, 110) });
      }
    }
  });
}
